...
while true
    ...
    %end
    k = [-8,-6,-4,-2,0,2];           % these are the given values
    lambda = 2.^k;                   %lamda is 2^k
    grad = zeros(n_vars, 1);         % initialize gradient
    grad = X_tr.'*(y_tr - (exp(X_tr*w))./(1+ exp(X_tr*w)))-(lambda.*w);
...
plot(k,acctrain,k,acctest);
legend('acctrain','acctest');